...
import {addUser, deleteUser, getUser, getUsers, updateUser, User} from "./repository";
import {generateUuid, isIdValid, isUserValid, parseBody, responses} from "./utils";
...
const server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  const createUsersController = async (params: string[]) => {
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.write(JSON.stringify(await getUser(userObj.id)));
  };
  ...
  const updateUsersController = async (params: string[]) => {
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.write(JSON.stringify(await getUser(userUuid)));
  };
